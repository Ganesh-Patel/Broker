...
import { updatListing } from '../../utils/listingApis.js'; // Update function for existing listing.
...
export default function EditListingModal({ isOpen, onClose, listingData }) {
  ...
  const handleImageSubmit = () => {
    if (Array.isArray(files) && files.length > 0 && files.length + formData.imageUrls.length <= 6) {
      setUploading(true);
      setImageUploadError('');
...
//       setFormData((prev) => ({
//         ...prev,
//         type: id, // Set type directly based on id
//       }));
//     } else {
//       setFormData((prev) => ({
//         ...prev,
//         [id]: checked !== undefined ? checked : e.target.value,
//       }));
//     }
//   };
...
const handleChange = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updatListing(updatedData, listingData._id);
